...
public class Lesson3 {
  ...
  static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
    ...
    IntStream intStream = parallel ? IntStream.range(0, LIST_SIZE).parallel() : IntStream.range(0, LIST_SIZE);
    distances = intStream
  ...
  static List<String> processWords(List<String> wordList, boolean parallel) {
    // YOUR CODE HERE
    final Stream<String> stream = parallel ? wordList.stream().parallel() : wordList.stream();
    System.out.println(stream.sorted(Comparator.comparingInt(String::length))
          .map(s -> s.toUpperCase())
          ...
          .sum());
    final Stream<String> stream2 = parallel ? wordList.stream().parallel() : wordList.stream();
    final List<String> sorted = stream2.sorted(Comparator.comparingInt(String::length))
          .map(s -> s.toUpperCase())
//          .peek(System.out :: println)
          .filter(s -> !s.startsWith("E"))
//          .peek(System.out :: println)
